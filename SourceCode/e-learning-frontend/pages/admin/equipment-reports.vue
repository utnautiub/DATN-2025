      ...
      <form @submit.prevent="createReport" class="space-y-4">
        ...
        <div>
          ...
          <select id="room_id" v-model="selectedRoomId" class="w-full px-4 py-2 border rounded-lg"
            @change="updateFilteredEquipmentItems">
        ...
        <div>
          ...
          <select id="equipment_item_id" v-model="newReport.equipment_item_id" required
            ...
            <option v-for="item in filteredEquipmentItems" :key="item.id" :value="item.id">
              {{ item.room_equipment?.name }} - {{ item.code }}
            </option>
          </select>
        </div>
        <div>
...
const filteredEquipmentItems = ref<EquipmentItem[]>([]);
...
const updateFilteredEquipmentItems = async () => {
  if (selectedRoomId.value === 0) {
    filteredEquipmentItems.value = [];
    return;
  }
  filteredEquipmentItems.value = equipmentItems.value.filter(item => item.room_equipment?.room?.id === selectedRoomId.value);
  console.log('Filtered equipment items:', filteredEquipmentItems.value);
};
...
const createReport = async () => {
  try {
    loading.value = true;
    error.value = '';
    newReport.value.status = 'Pending';
...
// Theo dõi thay đổi của selectedRoomId để cập nhật danh sách thiết bị
watch(selectedRoomId, () => {
  updateFilteredEquipmentItems();
});
